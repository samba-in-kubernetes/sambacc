...
class KeyBridge:
    ...
    def Get(
        ...
        represenation or encoding of the returned string. A scope is free to
        convert a value to a given kind or reject the requrest due to the kind
...
class VerifyPeerScopeWrapper:
    ...
    wrap a basic cope with the VerifyPeerScopeWrapper and let it
    ...
    def verify_peer(self, op: OpKind, pid: int, uid: int, gid: int) -> bool:
        ...
        if self._allowed_uids and uid not in self._allowed_uids:
            return False
        if self._allowed_gids and uid not in self._allowed_gids:
            return False
        return True
